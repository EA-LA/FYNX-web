...
async function load(kind,window,product){
 const key=kind==='correlation'?`fx-${window}`:product,old=cache.get(key),ttl=kind==='correlation'?21600000:10000;
 ...
 const data={...result,fetchedAt:new Date().toISOString(),stale:kind==='correlation'&&Date.now()-Date.parse(result.observationDate)>7*86400000};cache.set(key,data);return data;
 }catch(error){recordFailure({category:'feed',operation:'feed-load',code:'unavailable',page:'/'});if(old&&Date.now()-Date.parse(kind==='correlation'?old.fetchedAt:old.observationTime)<(kind==='correlation'?86400000:60000))return {...old,stale:true};throw error;}finally{pending.delete(key);}})();
...
exports.webMarketAnalytics=functions.runWith({timeoutSeconds:30,memory:'256MB',maxInstances:2}).https.onRequest(async(req,res)=>{
 ...
 if(!['correlation','liquidity'].includes(kind)||![30,60,90].includes(window)||!['BTC-USD','ETH-USD'].includes(product))return res.status(400).json({error:'Unsupported analysis'});
 try{const data=await load(kind,window,product);if(!data.stale)res.set('Cache-Control',kind==='correlation'?'public,max-age=300':'public,max-age=5');return res.json(data);}catch{return res.status(503).json({error:'Source data is unavailable. Retry shortly.'});}
});
