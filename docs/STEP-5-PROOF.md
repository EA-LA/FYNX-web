...
## First-party rollout completed for the six Risk calculators — September 22

Position size, risk/reward, pip value, margin, break-even and ATR stop now have an explicit authenticated test-API verification button. Local calculation remains usable independently. Each adapter forwards the inputs for its supported model and compares at displayed precision. All 16 adapter fixtures passed against the deployed HTTP API; see `proof/live-pilot-parity.json`.

The adapters do not certify broker instruments. Pip/margin use a clearly disclosed generic Forex model label; break-even keeps its USD-quoted, 100,000-unit contract assumption; position size keeps the local 0.01-lot increment and requires explicit cross-currency conversion. Inverse conversion and per-lot commission normalize to 12 decimal places, with mismatches shown for review. ATR verification is limited to the three Forex choices; gold/crypto verification is rejected.

The existing `apiPilot` claim now permits exactly those six calculator paths and no administrative routes. No person was granted a new claim. Set `riskRewardEnabled` false to pause risk/reward; remove other endpoint names from `enabledCalculators` in `api/first-party-pilot.json` to pause them individually. Calls always use the test environment. Input, side, preset and reset changes invalidate a pending verification result.

The new public `api/examples/prop-firm-quickstart.mjs` passed against the live test API. It verifies synthetic rules/account creation, ordered events, identical retries without another usage charge, changed-payload rejection, account status and linked audit records. A normal run retains a clearly named test account/rule set and consumes six successful test calls. The internal verification harness cleaned up its own QA records. This is preparation for external beta, not evidence that external developers have integrated.

Verification: existing proof checks, six-route access checks, 16 adapter-to-engine comparisons, controller sign-in/kill-switch/validation/mismatch/stale-response checks, API page checks and static production build all passed. Browser QA used isolated local rendering fixtures, preserving production access controls; all six pages fit 390px and 1440px viewports, and the signed-out verification button requests sign-in. The adapter requests and both Node quickstarts were tested against the actual backend with a disposable test account.

Current data inventory was checked again: five challenges, zero trades, zero rule evaluations; all five latest shadow outcomes are `awaiting_history`. The real-history, seven-day observation, purchased-terms reconciliation, Funded automatic migration and external beta gates remain incomplete.
