...
document.addEventListener('click', (e) => {
  ...
  // If clicking outside all bars AND not on mobile drawer/overlay
...
function openMobileMenu() {
  // close desktop bars so they don't conflict
  closeAllBars();
...
document.querySelectorAll('.m-accordion > .m-acc-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // Optional: close other top-level accordions (keep nested ones independent)
...
// Close drawer when user clicks any link (so it feels like a real app)
...
(function(){
  ...
  // ✅ IMPORTANT FIX:
  // Remove the "hide forever" localStorage behavior.
  // Widget ALWAYS returns on refresh / revisit.

  // Close button should not navigate (hide only this session)
  ...
  // Generate QR for the App Store link
  qrHost.innerHTML = "";
  if (window.QRCode) {
    new QRCode(qrHost, {
      text: APP_URL,
  ...
  // Show when scrolling DOWN, hide when scrolling UP, hide near top.
  ...
  function applyInitialState(){
    ...
    // Mobile: hidden until user scrolls down
    widget.classList.add("is-hidden");
  }
